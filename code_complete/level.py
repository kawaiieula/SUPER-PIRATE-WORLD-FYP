...
class Level:
    def __init__(self, tmx_map, level_frames, audio_files, data, switch_stage):
        ...
        # pause screen
        self.pause_screen = PauseScreen(self.unpause, self.exit_to_overworld)
        self.paused = False
        ...
        # Level notification setup
        self.font = pygame.font.Font('../font/horizon.otf', 40)
        self.level_name = tmx_level_properties.get('level_name', f'Level {self.data.current_level}')
        self.show_level_name = True
        self.level_name_timer = 3  # Show for 3 seconds
        self.level_name_surface = self.font.render(self.level_name, True, (255, 222, 32))  # Gold color
        self.level_name_rect = self.level_name_surface.get_rect(midtop=(WINDOW_WIDTH // 2, 50))

        # Congratulation notification setup
        self.congrats_font = pygame.font.Font('../font/horizon.otf', 32)  # Reduced font size for better fit
        self.show_congrats = False
        self.congrats_timer = 5  # Show for 5 seconds
        self.congrats_text = "Congratulations!"  # Split into two lines for better layout
        self.congrats_text2 = "You have unlocked The Captain!"
        self.congrats_surface = self.congrats_font.render(self.congrats_text, True, (255, 222, 32))
        self.congrats_surface2 = self.congrats_font.render(self.congrats_text2, True, (255, 222, 32))
        
        # Center both text lines
        self.congrats_rect = self.congrats_surface.get_rect(centerx=WINDOW_WIDTH // 2, centery=WINDOW_HEIGHT // 3)
        self.congrats_rect2 = self.congrats_surface2.get_rect(centerx=WINDOW_WIDTH // 2, centery=self.congrats_rect.bottom + 40)
        
        # Load character preview image with error handling
        try:
            self.character_preview = pygame.image.load('../graphics/newCharacter/pirataTestTwitter.gif').convert_alpha()
            # Keep original size of the GIF
            self.character_rect = self.character_preview.get_rect(midtop=(WINDOW_WIDTH // 2, self.congrats_rect2.bottom + 40))
        except FileNotFoundError:
            print("Captain GIF image not found. Using text-only congratulation.")
    ...
    def check_constraint(self):
        ...
        if self.player.hitbox_rect.colliderect(self.level_finish_rect):
            if self.data.current_level == 5:  # Assuming level 5 is the final level
                self.show_congrats = True
                self.congrats_timer = 5
                self.paused = True  # Pause the game when showing congratulation
            else:
                self.switch_stage('overworld', self.level_unlock)

    def pause_game(self, background):
        self.paused = True
        self.pause_screen.bg = background
    ...
    def handle_event(self, event):
        # Add any event handling specific to the Level class
        pass

    def run(self, dt):
        if not self.paused or self.show_congrats:  # Allow congrats to show even when paused
            ...
            # Draw level name notification
            if self.show_level_name:
                self.level_name_timer -= dt
                if self.level_name_timer <= 0:
                    self.show_level_name = False
                else:
                    # Create a semi-transparent background for the text
                    padding = 20
                    bg_rect = pygame.Rect(
                        self.level_name_rect.left - padding,
                        self.level_name_rect.top - padding,
                        self.level_name_rect.width + (padding * 2),
                        self.level_name_rect.height + (padding * 2)
                    )
                    bg_surface = pygame.Surface((bg_rect.width, bg_rect.height), pygame.SRCALPHA)
                    bg_surface.fill((0, 0, 0, 180))  # Semi-transparent black
                    self.display_surface.blit(bg_surface, bg_rect)
                    self.display_surface.blit(self.level_name_surface, self.level_name_rect)

            # Draw congratulation notification
            if self.show_congrats:
                self.congrats_timer -= dt
                if self.congrats_timer <= 0:
                    ...
                    # Draw both lines of congratulation text
                    self.display_surface.blit(self.congrats_surface, self.congrats_rect)
                    self.display_surface.blit(self.congrats_surface2, self.congrats_rect2)
                    
                    # Draw character preview only if image was loaded successfully
                    if self.character_preview:
                        self.display_surface.blit(self.character_preview, self.character_rect)